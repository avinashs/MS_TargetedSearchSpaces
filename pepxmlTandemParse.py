...
titleLine = "\t".join(["peptide","modpeptide","specid","mass","charge","hyperscore","nextscore","expect","iniprob","isfwd","protidString"]);
out.write(titleLine+"\n");
...
for pepxrec in pepxitr:
	...
	hyperscore = pepxrec['search_hit'][0]['search_score']['hyperscore'];
	nextscore = pepxrec['search_hit'][0]['search_score']['nextscore'];
	expect = pepxrec['search_hit'][0]['search_score']['expect'];
	...
	outstring = "\t".join(map(str,[peptide,modpeptide, specid, mass, charge, hyperscore, nextscore, expect, iniprob, isfwd, protidString]));
